...
function installLibreOffice() {
  ...
  // Non-Windows: open the download page.
...
// Pick the Prisma query engine for this platform (Windows build ships the
// windows engine; fall back to the first engine found).
function findEngine(dir) {
  try {
    const files = fs.readdirSync(dir).filter((n) => /query_engine/i.test(n) && /\.node$/i.test(n));
    if (!files.length) return undefined;
    const match = process.platform === "win32" ? files.find((f) => f.includes("windows")) : files[0];
...
app.whenReady().then(() => {
  Menu.setApplicationMenu(null);   // everything is driven from the in-app UI
  start().catch((err) => {
